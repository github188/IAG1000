...
/** 
 *   @brief     在虚拟设备中将一个用户地址信息加入用户列表
 *   @param  virdev_no虚拟设备号
 *   @param  guest 新用户的ip地址和端口号信息
 *   @return   0表示成功 1表示忙 负值表示出错
 *   @warnning 调用此函数前调用者应该已经取得了媒体服务的s_mutex
 */ 
...
/** 
 *   @brief     从虚拟设备音视频上行服务中删除指定的用户
 *   @param  virdev_no虚拟设备号
 *   @param  usr音视频上行用户信息结构指针
 *   @return   0表示成功 1表示忙 负值表示出错
 *   @warnning 调用此函数前调用者应该已经取得了媒体服务的s_mutex
 */ 
    ...
    while(1)
    {
		...
		*/
        ...
        while(1)
        {
			...
			else if(sel==0&&usr->serv_stat<0) //不发起信令只链接就滚蛋
				break;
