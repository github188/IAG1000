...
static int proc_audio_subscrib_cmd(IO aplay_usr_t *usr,IN struct gt_usr_cmd_struct* cmd)
{
    ...
    strncpy(usr->name,subscribe->account,20);   ///<用户名最长为19
    usr->name[19]='\0';
...
static void *rtnet_aplay_thread(void *para)
{
    ...
    unsigned char             play_buf[sizeof(net_rec_buf)*4];      ///<播放音频数据的缓冲区
    int                             play_buf_size=sizeof(play_buf);      ///<播放缓冲区的长度
...
HANDLE:
						if(ret>0)
						{
							...
							if(ret=write_frame_data(net_rec_buf,ret2+AAC_HEAD_SIZE*2)>0) //空实现
