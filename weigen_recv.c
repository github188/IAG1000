...
#define NETLINK_TEST 4 //自定义的协议
void * weigen_recv_thread(void* arg )
{
    int state;
    ...
    dest_addr.nl_pid = 0; //B：设置目的端口号 0:内核
...
int create_weigen_server(void)
{
	int                         i;
	pthread_t					thread_id;
	gt_create_thread(&thread_id,weigen_recv_thread,NULL);
	return 0;
}
